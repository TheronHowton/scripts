...
Function Test-ScheduleTask {
    ...
    $Task = $Schedule.GetFolder("\Event Viewer Tasks").GetTasks(0) | Where-Object { $_.Name -like $Name -and $_.Enabled -eq $true };
...
If (!(Test-Path $ScriptsFilePath\$SolutionFolder)) {
    New-Item -ItemType Directory -Path $ScriptsFilePath\$SolutionFolder -Force | Out-Null;
    Write-Log -File $LogFile -Status Information -Text "'$ScriptsFilePath' scripts folder created.";
}
...
If (Test-Path $ScriptsFilePath) {
    ...
    Write-Log -File $LogFile -Status Information -Text "'$ScriptsFilePath' scripts folder ACLed.";
}
...
$Path = $ScriptsFilePath + "\" + $SolutionFolder;
